 ...
 * Returns team picks
 */
export function getPicks(id: number): Promise<TeamPick[]> {

  return fromCache(`team-picks-${id}`, () => {
    return new Promise((resolve: any, reject: any) => {
      dataService.getEntry(id).then((data) => {
        setPicks(data).then((picks) => {
          resolve(picks);
        });
      });
    });
 ...
 * Returns team summary
 */
export function getStats(id: number): Promise<TeamStatistics> {
  return fromCache(`team-stats-${id}`, () => {
    return new Promise((resolve: any, reject: any) => {
      Promise.all([dataService.getEntry(id), getPicks(id)]).then((data) => {
        const stats = setStats(data[0], data[1]);
        resolve(stats);
      });
    });
  });
